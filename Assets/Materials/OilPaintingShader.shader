Shader "Unlit/OilPaintingShader"
{
    Properties
    {
        ...
        _StrokeHSV ("笔刷HSV", Vector) = (0.48,2.0,1.6)
        
        _RandomSeed ("笔刷分布随机种子",Range(-1000,1000)) = 0
        _StrokeSizeVariation ("笔刷大小",float) = 0.3
        _NoiseScale1 ("噪声大小1",float) = 1.0
        _NoiseScale2 ("噪声大小2",float) = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 texture_vector1,texture_vector2;
                float stroke_aplha1,stroke_aplha2;
                flood_fill_stroke(i.uv,_NoiseScale1,_RandomSeed,_StrokeSizeVariation,texture_vector1,stroke_aplha1);
                flood_fill_stroke(i.uv,_NoiseScale2,_RandomSeed,_StrokeSizeVariation,texture_vector2,stroke_aplha2);
                float3 base_color = tex2D(_MainTex,i.uv)*_BaseColor.rgb;
                float3 base_color1 = tex2D(_MainTex,texture_vector1);
                float3 base_color2 = tex2D(_MainTex,texture_vector2);
                float3 colorA = base_color;
                float3 colorB = lerp(base_color2,base_color1,stroke_aplha1)*_BaseColor.rgb;
                colorB = AdjustHSV(colorB,_StrokeHSV.x,_StrokeHSV.y,_StrokeHSV.z);
                float factor = color_screen(stroke_aplha1,stroke_aplha2)*0.6;
                fixed4 col = float4(lerp(colorA,colorB,factor),1.0);
                return col;
            }
            ENDCG
        }
